...
CRITICAL: You MUST include score_breakdown and dscr_calculation in your response. The score_breakdown must show the actual points awarded for each component based on the rubric above — do not leave any field as 0 unless the actual calculated points are 0. The dscr_calculation must show the actual numbers extracted from the documents. If you cannot calculate a field precisely, make your best estimate based on the documents provided and explain in the label. Never return default zero values — always return your actual calculated values. Example score_breakdown: {base_score: 50, dscr_points: 8, dscr_points_label: 'DSCR of 1.21x — below SBA minimum 1.25x, awarded 8 points for being above 1.15x', revenue_points: 15, revenue_points_label: 'Revenue growing from $228,400 to $235,000 consistently', cash_flow_points: 5, cash_flow_points_label: '1 negative month in December 2025', debt_load_points: 5, debt_load_points_label: 'Moderate debt load at 32% of gross revenue', document_quality_points: 0, document_quality_points_label: 'All 3 documents provided but all self-prepared, no CPA review', business_history_points: 5, business_history_points_label: '4+ years in operation estimated from documents', red_flag_deductions: [{flag_name: 'DSCR below SBA minimum', points: -5, reason: 'SBA SOP 50 10 8 requires minimum 1.25x DSCR'}, {flag_name: 'Revenue discrepancy >5%', points: -5, reason: 'Bank deposits $247,500 vs P&L revenue $235,000 — 5.3% gap exceeds threshold'}, {flag_name: 'Self-prepared financials', points: -3, reason: 'No CPA review on any document'}, {flag_name: 'Missing 2+ years tax returns', points: -5, reason: 'Only 2024 return provided, SBA requires 2-3 years'}, {flag_name: 'High-interest revolving debt', points: -3, reason: 'Chase Ink $8,340 at 19.99% APR'}, {flag_name: 'Negative cash flow month', points: -3, reason: 'December 2025 was -$3,500'}], final_score: 62}

Example dscr_calculation: {net_operating_income: 32595, addbacks: ['Owner salary $72,000 added back — SBA allows owner compensation addback for single-owner businesses', 'Equipment loan payment $19,440 added back — already deducted in P&L operating expenses', 'SBA microloan payment $5,760 added back — already deducted in P&L operating expenses'], adjusted_income: 129795, total_annual_debt_service: 25200, debt_items: ['Equipment loan: $1,620/month x 12 = $19,440/year', 'SBA microloan: $480/month x 12 = $5,760/year', 'Credit card minimum: $250/month x 12 = $3,000/year'], dscr_formula: '$129,795 adjusted income ÷ $25,200 annual debt service = 1.29x', sba_minimum: 1.25, meets_minimum: true}`;
